...
public class UnTreatedAdapter extends BaseQuickAdapter<UnWaiMaiEntity.DataBean , BaseViewHolder> {

    public UnTreatedAdapter(@LayoutRes int layoutResId, @Nullable List<UnWaiMaiEntity.DataBean> data) {
        super(layoutResId, data);
    }

    @Override
    protected void convert(BaseViewHolder helper, UnWaiMaiEntity.DataBean item) {
        String date = "";
        try {
        ...
        for (int i = 0; i < item.getProductList().size(); i++) {
            UnWaiMaiEntity.DataBean.ProductListBean productListBean1 = item.getProductList().get(i);
        ...
        switch (item.getOrderState()){
            case 1:
                helper.setText(R.id.tv_jiedan , "接单");
